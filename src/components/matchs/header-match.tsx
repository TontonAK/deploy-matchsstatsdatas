...
export default function HeaderMatch({ matchData }: HeaderMatchProps) {
  return (
    <header className="pt-15 lg:pt-11 bg-gradient-to-r from-gradient-start to-plaisir-primary lg:bg-gradient-to-t lg:from-gradient-start lg:to-plaisir-primary flex flex-col lg:items-center relative w-full z-0 px-6 lg:px-0 pb-6 lg:pb-20 uppercase">
      ...
      {matchData.match.seasonLeagueMatch?.seasonLeague.gameDay && (
        <div className="flex flex-row text-xs justify-center lg:text-xl text-white py-2 z-10">
          <span>
            Journée {matchData.match.seasonLeagueMatch?.seasonLeague.gameDay}
          </span>
        </div>
      )}
      <div className="flex flex-col lg:flex-row text-xs lg:text-sm text-white pb-4 pt-6 lg:pt-6 lg:pb-2 z-10 space-x-4 border-b border-solid border-white lg:border-none gap-y-4 lg:gap-y-0">
      ...
      <div className="grid grid-rows-6 grid-cols-12 lg:flex-row lg:gap-2 text-white pt-10 lg:pb-8 z-10 lg:px-20 w-full">
        <div className="col-start-1 col-span-10 row-start-1 row-span-2 flex flex-row lg:justify-center lg:flex-col lg:col-start-1 lg:col-span-4 lg:row-start-1 lg:row-span-6 items-center uppercase">
          <TileTeamMatch
        ...
        <div className="col-start-11 col-span-2 lg:col-span-2 flex row-start-1 row-span-2 lg:col-start-5 justify-center lg:row-start-1 lg:row-span-5 items-center text-4xl lg:text-6xl">
          <div className="flex items-center py-2 rounded-lg font-bold">
            {matchData.match.scoreHomeTeam}
          </div>
        </div>
        <div className="flex col-start-11 col-span-2 row-start-3 row-span-2 lg:col-start-7 lg:col-span-2 lg:row-start-1 lg:row-span-5 items-center justify-center text-4xl lg:text-6xl">
          <div className="flex items-center py-2 rounded-lg font-bold">
            {matchData.match.scoreAwayTeam}
          </div>
        </div>
        <div className="col-start-1 col-span-10 row-start-3 row-span-2 flex flex-row lg:justify-center lg:flex-col lg:col-start-9 lg:col-span-4 lg:row-start-1 lg:row-span-6 items-center uppercase">
          <TileTeamMatch
      ...
      {matchData.match.halfTimeScore && (
        <div className="font-light text-white col-start-1 col-span-12 row-start-5 row-span-1 lg:col-start-4 lg:col-span-6 lg:row-start-5 lg:row-span-1 text-xs lg:text-lg font-montserrat text-center pt-2 lg:pt-3">
          Mi-temps : {matchData.match.halfTimeScore?.homeScore} -{" "}
          {matchData.match.halfTimeScore?.awayScore}
        </div>
      )}
    </header>
  );
