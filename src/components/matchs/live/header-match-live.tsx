...
import { startLiveAction } from "../../../../app/(connected-layout)/matchs/[ulid]/live/start-live.action";
import Stopwatch from "../../../../app/(connected-layout)/matchs/[ulid]/live/stopwatch";
import { TileTeamMatch } from "../tile-team-match";

interface HeaderMatchLiveProps {
  matchData: MatchStats;
  onLiveStarted: () => void;
  onMatchFinished?: () => void;
}

export default function HeaderMatchLive({
  matchData,
  onLiveStarted,
  onMatchFinished,
}: HeaderMatchLiveProps) {
  const { execute: startLive, isPending } = useAction(startLiveAction, {
    onSuccess: () => {
  ...
  const handleStartLive = () => {
    startLive({ matchId: matchData.matchId });
  };
  ...
  return (
    <header className="pt-11 bg-gradient-to-r from-gradient-start to-plaisir-primary lg:bg-gradient-to-t lg:from-gradient-start lg:to-plaisir-primary flex flex-col lg:items-center relative w-full z-0 px-6 lg:px-0 pb-6 lg:pb-20 uppercase">
      ...
      <div className="grid grid-rows-6 grid-cols-12 lg:flex-row lg:gap-2 text-white pt-10 lg:pb-8 z-10 lg:px-20 w-full">
        ...
        <div className="col-start-11 col-span-2 lg:col-span-2 flex row-start-1 row-span-2 lg:col-start-6 lg:justify-center lg:row-start-1 lg:row-span-6 items-center flex-col gap-6">
      ...
      {/* Bouton Démarrer live ou Chrono */}
      {!isLive ? (
        <div className="flex flex-col items-center justify-center">
          <AlertDialog>
            ...
            <AlertDialogContent>
              <AlertDialogHeader>
                <AlertDialogTitle>Démarrer le live du match</AlertDialogTitle>
                <AlertDialogDescription>
                  Êtes-vous sûr de vouloir démarrer le live de ce match ? Une
                  fois le live lancé, vous devrez rester sur cette page tout au
                  long du match pour enregistrer les événements.
                </AlertDialogDescription>
              </AlertDialogHeader>
              <AlertDialogFooter>
        ...
        <Stopwatch
          matchPeriod={matchData.match.periodType}
          matchId={matchData.matchId}
          onMatchFinished={onMatchFinished ?? (() => undefined)}
        />
      )}
    </header>
  );
