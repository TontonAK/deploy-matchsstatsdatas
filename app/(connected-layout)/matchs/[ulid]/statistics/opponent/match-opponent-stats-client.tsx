...
import { MatchOpponentStatsResult } from "@/database/statistics/get-match-opponent-stats";
import { StatTypeGamePhase, StatValueType } from "@/generated/prisma";
import { useRouter } from "next/navigation";
import { useState } from "react";
import { updateMatchOpponentStatsAction } from "./update-opponent-stats.action";

interface MatchOpponentStatsClientProps {
  opponentStats: MatchOpponentStatsResult;
  matchId: number;
  opponentTeamId: number;
}

export function MatchOpponentStatsClient({
  opponentStats,
  matchId,
  opponentTeamId,
}: MatchOpponentStatsClientProps) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const result = await updateMatchOpponentStatsAction({
        matchId,
        opponentTeamId,
        stats: statsToUpdate,
      });
  ...
  const getDisplayValue = (stat: (typeof opponentStats.stats)[0]) => {
    if (
      isEditMode &&
  ...
  return (
    <div className="space-y-8">
      ...
      {/* Informations de l'équipe adverse */}
      <div className="mb-6">
        <h2 className="text-xl font-semibold mb-2">
          Statistiques de {opponentStats.clubName}
        </h2>
      </div>
